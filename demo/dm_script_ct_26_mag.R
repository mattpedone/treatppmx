...
#for(kk in 1:KK){
  ...
  for(j in 1:mydata$nclus[1]){
    cat(apply(matrix(Y[which(mydata$labeltrain == j & mydata$treat=="A"),], ncol=ncol(Y)), 2, sum), "\n")
  }

  for(j in 1:mydata$nclus[2]){
    cat(apply(matrix(Y[which(mydata$labeltrain == j & mydata$treat=="B"),], ncol=ncol(Y)), 2, sum), "\n")
  }
  ...
  ppm <- postquant_dm_ct(y = Ytrain, yp = Ytest, output = out_ppm,
                      data = mydata, plot = F)
  ppm$nclupost
  ppm$ARI_B
  ppm$ARI_VI

  sam <- sample(1:100, 10, T)
  ppm$pipredl$A[sam,]
  ppm$pipredl$B[sam,]
  df <- data.frame(pred = ppm$mypred, truth = mydata$treattest[sam]); df

  ppm$pipredl$A[41:45,]
  ppm$pipredl$B[41:45,]
  Ytest[41:45,]
  mydata$treattest[41:45]
