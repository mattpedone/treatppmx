...
KK <- 15#numero di repliche
res <- matrix(0, KK, 8)
outtab <- array(0, dim = c(13, 8, KK))
##list for aucs
roc_res_ppmx0_aux <- roc_res_ppmx0_aux_IG <- list()
roc_res_ppmx0_dd <- roc_res_ppmx0_dd_IG <- list()
roc_res_ppmx1_aux <- roc_res_ppmx1_aux_IG <- list()
roc_res_ppmx1_dd <- roc_res_ppmx1_dd_IG <- list()
roc_res_ppmx2_aux <- roc_res_ppmx2_aux_IG <- list()
roc_res_ppmx2_dd <-roc_res_ppmx2_dd_IG <- list()
...
time_ppmx0_aux <- system.time(
  out_ppmx0_aux <- my_dm_ppmx(y = Ytrain, X = Xtrain, Xpred = Xtest,
                              z = Ztrain, zpred = Ztest, alpha=alpha_DP,
                              CC = n_aux, reuse = 1,
...
tab <- cbind(tab[,-c(5,6)], time)
#kk=1
outtab[,,kk] <- matrix(data.matrix(unlist(tab)), ncol=8)

roc_res_ppmx0_aux[[kk]] <- compute_auc(out_ppm, out_ppmx0_aux)
roc_res_ppmx0_aux_IG[[kk]] <- compute_auc(out_ppm, out_ppmx0_aux_IG)
roc_res_ppmx0_dd[[kk]] <- compute_auc(out_ppm, out_ppmx0_dd)
roc_res_ppmx0_dd_IG[[kk]] <- compute_auc(out_ppm, out_ppmx0_dd_IG)
roc_res_ppmx1_aux[[kk]] <- compute_auc(out_ppm, out_ppmx1_aux)
roc_res_ppmx1_aux_IG[[kk]] <- compute_auc(out_ppm, out_ppmx1_aux_IG)
roc_res_ppmx1_dd[[kk]] <- compute_auc(out_ppm, out_ppmx1_dd)
roc_res_ppmx1_dd_IG[[kk]] <- compute_auc(out_ppm, out_ppmx1_dd_IG)
roc_res_ppmx2_aux[[kk]] <- compute_auc(out_ppm, out_ppmx2_aux)
roc_res_ppmx2_aux_IG[[kk]] <- compute_auc(out_ppm, out_ppmx2_aux_IG)
roc_res_ppmx2_dd[[kk]] <- compute_auc(out_ppm, out_ppmx2_dd)
roc_res_ppmx2_dd_IG[[kk]] <- compute_auc(out_ppm, out_ppmx2_dd_IG)
...
#average aucs
avg_roc <- avg_auc(roc_res_ppmx0_dd, KK)

p <- my_plot_multi_roc_data(avg_roc)
ggplot(p, aes(x = 1-Specificity, y=Sensitivity)) +
