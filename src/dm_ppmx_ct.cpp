...
Rcpp::List dm_ppmx(int iter, int burn, int thin, int nobs, arma::vec treatments, int PPMx, int ncon, int ncat,
  ...
  // t - treatments index
  int l, ll, lll, i, ii, iii, c, p, pp, j, jj, mm, zi, k, q, h, t;
  int dim = y.n_cols;
  int Q = z.n_cols;
    ...
    for(t = 0; t < nT; t++){
      for(p = 0; p < ncon; p++){
        for(i = 0; i < nobs; i++){
          if(treatments(i) == t){
            sum(t) += xcon(i*(ncon) + p);
            sum2(t) += xcon(i*(ncon) + p)*xcon(i*(ncon) + p);
          }
        }
        //Rcpp::Rcout << "sum" << sum << std::endl;
        xbar(t, p) = sum(t)/((double) num_treat(t));
        s2mle(t, p) = sum2(t)/((double) num_treat(t)) - xbar(t, p)*xbar(t, p);
      }
    }
  }
  ...
  for(t = 0; t < nT; t++){
    for(i = 0; i < num_treat(t); i++){
      curr_clu(t, i) = i + 1;//R::rbinom(1, 0.5) + 1;
    }
  }
  ...
  for(t = 0; t < nT; t++){
    for(i = 0; i < num_treat(t); i++){
      for(ii = 0; ii < num_treat(t); ii++){
        if(curr_clu(t, i) == ii+1) nj_curr(t, ii) += 1;
      }
    }
  }
  ...
  for(t = 0; t < nT; t++){
    for(i = 0; i < num_treat(t); i++){
      if(nj_curr(t, i) > 0) nclu_curr(t) += 1;
    }
  }
  ...
  double vp;
  for(t = 0; t < nT; t++){
    vp = pow(num_treat(t) + CC, -1.0);
    weight.row(t).fill(vp);
  }
  ...
  for(t = 0; t < nT; t++){
    for(i = 0; i < nobs; i++){
      if(treatments(i) == t){
        clu_lg = curr_clu(t, i)-1;
        for(k = 0; k < dim; k++){
          loggamma(i, k) = calculate_gamma(eta_star_curr.slice(t), z, beta, clu_lg, k, i, 1);
        }
      }
    }
  }
  ...
  if(PPMx == 1){
    ...
    for(t = 0; t < nT; t++){
      for(i = 0; i < nobs; i++){
        if(treatments(i) == t){
          for(p = 0; p < ncon; p++){
            sumx(t, (curr_clu(i)-1)*ncon + p) += xcon(i * ncon + p);
            sumx2(t, (curr_clu(i)-1)*ncon + p) += xcon(i * ncon + p) * xcon(i * ncon + p);
          }
          for(p = 0; p < ncat; p++){
            njc(t, ((curr_clu(i)-1)*ncat + p) * max_C + xcat(i * ncat + p)) += 1;
          }
        }
      }
    }
  ...
  for(l = 0; l < iter; l++){
    for(t = 0 ; t < nT; t++){
      ...
      if(reuse == 1){
        for(mm = 0; mm < CC; mm++){
          eta_empty.slice(t).col(mm) = ran_mvnorm(hP0_m0, hP0_L0, dim);
        }
      }
      ...
      for(i = 0; i < nobs; i++){
        if(treatments(i) == t){
          ...
          /////////////////////////////////////////
          zi = curr_clu(t, i)-1; //sottraggo 1 perché C conta da 0 ma conviene farlo ogni volta

          if(nj_curr(t, zi) > 1){// Case where the nj corresponding to zi is such that nj>1
            nj_curr(t, zi) -= 1;
            if(PPMx == 1){
              // need to reduce the sumx sumx2 to
              for(p = 0; p < ncon; p++){
                sumx(t, zi * ncon + p) -= xcon(i * ncon + p);
                sumx2(t, zi * ncon + p) -= xcon(i * ncon + p) * xcon(i * ncon + p);
              }

              // need to reduce the nhc
              for(p = 0; p < ncat; p++){
                njc(t, (zi * ncat + p) * max_C + xcat(i * ncat + p)) -= 1;
              }
            }
            ...
            if(iaux < nclu_curr(t)){
              for(ii = 0; ii < num_treat(t); ii++){
                if(curr_clu(t, ii) == nclu_curr(t)){
                  curr_clu(t, ii) = iaux;
                }
              }
              curr_clu(t, i) = nclu_curr(t);

              auxv = eta_star_curr.slice(t).col(zi);
              eta_star_curr.slice(t).col(zi) = eta_empty.slice(t).col(id_empty);
              eta_empty.slice(t).col(id_empty) = auxv;

              nj_curr(t, zi) = nj_curr(t, nclu_curr(t)-1);
              nj_curr(t, nclu_curr(t)-1) = 1;

              if(PPMx == 1){
                // need to swap sumx and sumx2
                for(p = 0; p < ncon; p++){
                  auxreal = sumx(t, (iaux-1)*(ncon) + p);
                  sumx(t, (iaux-1)*ncon + p) = sumx(t, (nclu_curr(t)-1)*(ncon) + p);
                  sumx(t, (nclu_curr(t)-1)*(ncon) + p) = auxreal;

                  auxreal = sumx2(t, (iaux-1)*(ncon) + p);
                  sumx2(t, (iaux-1)*(ncon) + p) = sumx2(t, (nclu_curr(t)-1)*(ncon) + p);
                  sumx2(t, (nclu_curr(t)-1)*(ncon) + p) = auxreal;
                }

                // need to swap nhc as well
                for(p = 0; p < ncat; p++){
                  for(c=0; c<max_C; c++){
                    auxint = njc(t, ((iaux-1)*(ncat) + p)*(max_C) + c);
                    njc(t, ((iaux-1)*(ncat) + p)*(max_C) + c) = njc(t, ((nclu_curr(t)-1)*(ncat) + p)*(max_C) + c);
                    njc(t, ((nclu_curr(t)-1)*(ncat) + p)*(max_C) + c) = auxint;
                  }
                }
              }
            }
            ...
            if(PPMx == 1){
              for(p = 0; p < ncon; p++){
                sumx(t, (nclu_curr(t)-1)*(ncon) + p) -= xcon(i*(ncon)+p);
                sumx2(t, (nclu_curr(t)-1)*(ncon) + p) -= xcon(i*(ncon)+p)*xcon(i*(ncon)+p);
              }

              // need to reduce the nhc
              for(p = 0; p < ncat; p++){
                njc(t, ((nclu_curr(t)-1)*(ncat) + p)*(max_C) + xcat(i*(ncat)+p)) -= 1;
              }
            }
            ...
            for(ii = 0; ii < nobs; ii++){
              if(treatments(ii) == t){
                if(ii != i){
                  clu_lg = curr_clu(t, ii) -1;
                  for(k = 0; k < dim; k++){
                    loggamma(ii, k) = calculate_gamma(eta_star_curr.slice(t).col(clu_lg),
                             z, beta, 0, k, ii, 1);
                  }
                }
              }
          ...
          for(j = 0; j < nclu_curr(t); j++){
            ...
            if(PPMx == 1){
              ...
              for(p = 0; p < (ncon); p++){
                sumxtmp = sumx(t, j * ncon + p);
                sumx2tmp = sumx2(t, j * ncon + p);
                //Rcpp::Rcout << "check1: " << nj_curr(j) << std::endl;
                //Rcpp::Rcout << "check2: " << xbar << std::endl;
                if(similarity==1){ // Auxilliary
                  if(consim==1){//normal normal
                    lgcont = gsimconNN(m0, v, s20, sumxtmp, sumx2tmp, xbar(t, p), nj_curr(t, j), 0, 0, 1);
                    lgconN += lgcont;
                  }
                  if(consim==2){//normal normal inverse gamma
                    lgcont = gsimconNNIG(m0, k0, nu0, s20, sumxtmp, sumx2tmp, xbar(t, p), s2mle(t, p), nj_curr(t, j), 0, 0, 1);
                    lgconN += lgcont;
                  }
                }
                if(similarity==2){ //Double Dipper
                  if(consim==1){//normal normal
                    lgcont = gsimconNN(m0, v, s20, sumxtmp, sumx2tmp, xbar(t, p), nj_curr(t, j), 1, 0, 1);
                    lgconN += lgcont;
                  }
                  if(consim==2){//normal normal inverse gamma
                    lgcont = gsimconNNIG(m0, k0, nu0, s20, sumxtmp, sumx2tmp, xbar(t, p), s2mle(t, p), nj_curr(t, j), 1, 0, 1);
                    lgconN += lgcont;
                  }
                }
                ...
                if(similarity==1){ // Auxilliary
                  if(consim==1){//normal normal
                    lgcont = gsimconNN(m0, v, s20, sumxtmp, sumx2tmp, xbar(t, p), nj_curr(t, j) + 1, 0, 0, 1);
                    lgconY = lgconY + lgcont;
                  }
                  if(consim==2){//normal normal inverse gamma
                    lgcont = gsimconNNIG(m0, k0, nu0, s20, sumxtmp, sumx2tmp, xbar(t, p), s2mle(t, p), nj_curr(t, j) + 1, 0, 0, 1);
                    lgconY = lgconY + lgcont;
                  }
                }
                if(similarity==2){ //Double Dipper
                  if(consim==1){//normal normal
                    lgcont = gsimconNN(m0, v, s20, sumxtmp, sumx2tmp, xbar(t, p), nj_curr(t, j) + 1, 1, 0, 1);
                    lgconY = lgconY + lgcont;
                  }
                  if(consim==2){//normal normal inverse gamma
                    lgcont = gsimconNNIG(m0, k0, nu0, s20, sumxtmp, sumx2tmp, xbar(t, p), s2mle(t, p), nj_curr(t, j) + 1, 1, 0, 1);
                    lgconY = lgconY + lgcont;
                  }
                }
              ...
              for(p = 0; p < ncat; p++){
                for(c = 0; c < max_C; c++){
                  njctmp(c) = njc(t, (j*ncat+p)*max_C+c);
                }
            ...
            weight(t, j) = log((double) nj_curr(t, j)) + // cohesion part
            ...
            for(k = 0; k < dim; k++){
              wo = calculate_gamma(eta_star_curr.slice(t), z, beta, j, k, i, 0);
              weight(t, j) += wo * log(JJ(i, k)) - lgamma(wo) - JJ(i, k) * (ss(i) + 1.0);
              if(y(i, k) != 1){
                weight(t, j) -=  log(JJ(i, k));
              }
            }
          ...
          lgcondraw = 0.0;
          lgcatdraw = 0.0;
          for(j = nclu_curr(t); j < (nclu_curr(t) + CC); j++){
            jj = j - nclu_curr(t);
            if(PPMx == 1){
              // Continuous Covariates
              for(p = 0; p < (ncon); p++){
                ...
                if(similarity==1){ // Auxilliary
                  if(consim==1){//normal normal
                    lgcont = gsimconNN(m0, v, s20, tmp, tmp*tmp, xbar(t, p), 1, 0, 0, 1);
                    lgcondraw += lgcont;
                  }
                  if(consim==2){//normal normal inverse gamma
                    lgcont = gsimconNNIG(m0, k0, nu0, s20, tmp, tmp*tmp, xbar(t, p), s2mle(t, p), 1, 0, 0, 1);
                    lgcondraw += lgcont;
                  }
                }
                if(similarity==2){ //Double Dipper
                  if(consim==1){//normal normal
                    lgcont = gsimconNN(m0, v, s20, tmp, tmp*tmp, xbar(t, p), 1, 1, 0, 1);
                    lgcondraw += lgcont;
                  }
                  if(consim==2){//normal normal inverse gamma
                    lgcont = gsimconNNIG(m0, k0, nu0, s20, tmp, tmp*tmp, xbar(t, p), s2mle(t, p), 1, 1, 0, 1);
                    lgcondraw += lgcont;
                  }
                }
              }//chiude ciclo su p covariate continue
            ...
            weight(t, j) = log(alpha) - log(CC) + //cohesion + auxiliary ptms
              lgcondraw + // Continuous covariate part
              lgcatdraw; // categorical covariate part
            for(k = 0; k < dim; k++){
              wo = calculate_gamma(eta_empty.slice(t), z, beta, jj, k, i, 0);
              weight(t, j) += wo * log(JJ(i, k)) - lgamma(wo) - JJ(i, k) * (ss(i) + 1.0);
              if(y(i, k) != 1){
                weight(t, j) -=  log(JJ(i, k));
              }
          ...
          if((calibration == 1) & (PPMx == 1)){
            ...
            for(j = 0; j < nclu_curr(t) + CC; j++){

              lgtilN(j) = gtilN(j) - maxgtilN;
              sgN = sgN + exp(lgtilN(j));

              if(j < nclu_curr(t)){// se è in un cluster non è un signoletto
            ...
            for(j = nclu_curr(t); j < nclu_curr(t) + CC; j++){
              ...
              weight(t, j) = log(alpha) - log(CC) +  // Cohesion part
                //lgtilYk -
                lgtilNk;
          ...
          maxwei = weight(t, 0);
          for(j = 1; j < nclu_curr(t)+ CC; j++){
            if(maxwei < weight(t, j)) maxwei = weight(t, j);
          }

          denwei = 0.0;

          for(j = 0; j < nclu_curr(t) + CC; j++){
            weight(t, j) = exp(weight(t, j) - maxwei);
            denwei += weight(t, j);
          }

          for(j = 0; j < nclu_curr(t) + CC; j++){
            pweight(t, j) = weight(t, j)/denwei;
            //mysws += pweight(j);
          }
           ...
           */
          if((newci) <= (nclu_curr(t))){
            //Point 2a page 345 Favaro and Teh, second column
            curr_clu(t, i) = newci;
            nj_curr(t, newci - 1) += 1;
            //Rcpp::Rcout << "if 1" << curr_clu.t() << std::endl;
            for(k = 0; k < dim; k++){
              loggamma(i, k) = calculate_gamma(eta_star_curr.slice(t),
                       z, beta, curr_clu(t, i)-1, k, i, 1);
            }
          }else{
            /*id_empty: it identifies which of the augmented has been chosen
             * 2b  page 345 Favaro and Teh, second column
             */
            id_empty = newci - nclu_curr(t) - 1;
            nclu_curr(t) += 1;
            curr_clu(t, i) = nclu_curr(t);
            nj_curr(t, nclu_curr(t)-1) = 1;

            eta_star_curr.slice(t).col(nclu_curr(t)-1) = eta_empty.slice(t).col(id_empty);
            ...
            //}
            eta_empty.slice(t).col(id_empty) = ran_mvnorm(hP0_m0, hP0_L0, dim);
          }

          if(PPMx == 1){
            // need to now add the xcon to the cluster to which it was assigned;
            for(p = 0; p < ncon; p++){
              sumx(t, (curr_clu(t, i)-1)*ncon + p) += xcon(i * ncon + p);
              sumx2(t, (curr_clu(t, i)-1)*ncon + p) += xcon(i * ncon + p) * xcon(i * ncon + p);
            }
            for(p = 0; p < ncat; p++){
              njc(t, ((curr_clu(t, i)-1)*ncat + p) * max_C + xcat(i * ncat + p)) += 1;
            }
          }
        }
      }
      ...
      for(j = 0; j < nclu_curr(t); j++){
        ...
        l_eta_out = eta_update(JJ, beta, z, loggamma, nclu_curr(t), curr_clu.row(t).t(), nj_curr.row(t).t(),
                               eta_star_curr.slice(t).col(j), eta_flag.row(t).t(), mu0, L0v, j, mhtunepar(0));
