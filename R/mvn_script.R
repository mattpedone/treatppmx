...
system.time(out <- my_mvn_ppmx(y = Y, X = X, alpha=1, CC = 3, reuse = 1, PPMx = 1, similarity = 1, consim=1, calibration=2, 
                   similparam=c(0.0, 1.0, 0.1, 10.0, 2.0, 0.1, 1.0), 
                   modelpriors, mhtune=c(0.5, 0.5), 
                   iter=1,burn=0,thin=1))

cls <- as.matrix(out$label)
psm <- comp.psm(cls)
mc <- minbinder.ext(psm)
adjustedRandIndex(mc$cl, myppmx$label)
mean(out$nclu)
