...
my_mvn_ppmx_bycol <- function(y,X=NULL, alpha=1, CC = 3, PPMx = 1, similarity = 1, consim=1, calibration=0,
                        similparam=c(0.0, 1.0, 0.1, 1.0, 2.0, 0.1, 1.0),
                        modelpriors,
                        mhtune=c(0.5, 0.5),
                        iter=1100,burn=100,thin=1){
  ...
  out <- mvn_ppmx_bycol(as.integer(iter), as.integer(burn), as.integer(thin), 
                  as.integer(nobs), as.integer(PPMx), as.integer(ncon), as.integer(ncat), 
                  as.vector(catvec), as.double(alpha), as.integer(CC), 
                  as.integer(consim), as.integer(similarity), 
                  as.integer(calibration), as.matrix(y), 
  ...
  res$nclu <- nclu
  nclu_cs <- cumsum(nclu)
  mu_out <- t(out$mu)#matrix(out$mu, nrow=nout*nobs, byrow=TRUE)
  mu_ar <- array(0, dim = c(max(nclu), ncol(y), nout))
  for(l in 1:nout){
    for(i in 1:nclu[l]){
      mu_ar[i, ,1] <- mu_out[i, ]
