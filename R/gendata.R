...
genoutcomeL <- function(nobs, alpha, beta1, beta2, beta3, metx, x2, x3){
  prob <- matrix(0, nrow = nobs, ncol = 3)
  myy <- rep(0, nobs)
...
genmech_hetL <- function(npred = 10, nset = 30, overlap = 0.8){
  ...
  for(i in 1:nset){
    train <- ttL(pred = pred[id_train,groups$g1], prog = prog[id_train,])
    test <- ttL(pred[id_test,groups$g2], prog[id_test,])
